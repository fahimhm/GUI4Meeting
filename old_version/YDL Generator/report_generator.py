...
for training in data['training'].unique():
    train_base = data[(data['training'] == training) & (data['status_report'].isnull()) & (data['fdh'] == 'done')]
    for date_start in train_base['waktu'].unique():
        ...
        table_2 = date_base[['peserta', 'dept', 'atasan', 'post_test', 'presensi', 'reason_absen']]
        ...
        for i in date_base.index:
            mycell = worksheet.cell(row=(i+2), column=21)
            mycell.value = 'done'
